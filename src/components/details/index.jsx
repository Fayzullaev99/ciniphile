...
function Details() {
    ...
    const [recommend] = useFetch("recommend",`movie/${params.id}/recommendations`)
    return (
            ...
            <div className={styles.details__box}>
                <div className={styles.details__about}>
                    ...
                    <div className={styles.details__info}>
                        <p className={styles.details__year}>{new Date(details?.release_date).getFullYear()},</p>
                    ...
                    <button className={styles.details__button} onClick={()=>navigate(`/thriller/${details?.id}`)}>
                        <FaPlay />
                        <span>Смотерть трейлер</span>
                    </button>
                </div>
                <div className={styles.details__image}>
                    <img src={`${image_url}/original${details?.backdrop_path}`} alt={details?.title} />
                </div>
                ...
                <img src={`${image_url}/original${details?.backdrop_path}`} alt={details?.title} className={styles.details__bg} />
            </div>
            <Container>
                ...
                <div className={styles.details__recommend}>
                    ...
                    <div className={styles.details__recommend_box}>
                        {recommend?.results?.slice(0,4).map((el)=>(
                            <Card key={el.id} image={el.poster_path} title={el.title} click={()=>navigate(`/details/${el.id}`)} />
                        ))}
                    </div>
                </div>
            </Container>
